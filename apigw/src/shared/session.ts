...
export interface LogoutTokens {
  save(req: express.Request, logoutToken?: LogoutToken['value']): Promise<void>
  refresh(req: express.Request): Promise<void>
  logout(token: LogoutToken['value']): Promise<unknown | undefined>
  consume(token: LogoutToken['value']): Promise<void>
}

export function logoutTokenSupport(
  redisClient: RedisClient,
  config: { sessionTimeoutMinutes: number }
): LogoutTokens {
  /**
  ...
  async function refresh(req: express.Request): Promise<void> {
    if (!req.session) return
  ...
  async function consume(token: LogoutToken['value']) {
