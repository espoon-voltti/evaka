...
export function apiRouter(config: Config, redisClient: RedisClient) {
  ...
  router.use((req, _, next) => {
    if (req.url === '/application/version' || req.url === '/internal/version') {
      ...
      req.url = req.url.replace('/application/map-api/', '/citizen/public/')
      ...
      req.url = req.url.replace('/internal/auth/saml/', '/auth/entraid/')
  ...
  router.all('/auth/entraid/*', (req: express.Request, _, next) => {
    // horrible hack to fix logout URL
    if (req.session?.idpProvider === 'evaka') {
      req.url = req.url.replace('/auth/entraid/', '/auth/keycloak-employee/')
    }
    next()
  })
