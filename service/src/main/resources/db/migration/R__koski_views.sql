...
DROP FUNCTION IF EXISTS koski_active_preschool_study_right(today date, sync_range_start date);
DROP FUNCTION IF EXISTS koski_active_preparatory_study_right(today date);
DROP FUNCTION IF EXISTS koski_active_preparatory_study_right(today date, sync_range_start date);
DROP FUNCTION IF EXISTS koski_voided_study_right(today date);
DROP FUNCTION IF EXISTS koski_placement(today date);
DROP FUNCTION IF EXISTS koski_placement(today date, sync_range_start date);
...
CREATE FUNCTION koski_placement(today date, sync_range_start date) RETURNS
TABLE (
...
BEGIN ATOMIC
    SELECT
        ...
        range_agg(daterange(start_date, end_date, '[]') * daterange(sync_range_start, null, '[]')) AS placements,
    ...
    FROM (
        ...
        AND (sync_range_start IS NULL OR sync_range_start <= placement.end_date)
        WINDOW
...
CREATE FUNCTION koski_active_preschool_study_right(today date, sync_range_start date) RETURNS
...
CREATE FUNCTION koski_active_preparatory_study_right(today date, sync_range_start date) RETURNS
