...
WITH child_age_period AS (
  ...
  SELECT
    ...
    ext.uuid_generate_v1mc() AS new_id,
  ...
  UPDATE placement orig SET start_date = p.start_date, end_date = p.end_date, type = p.type
  FROM split_placement p
  ...
  UPDATE daycare_group_placement orig SET daycare_placement_id = gp.new_id, start_date = gp.start_date, end_date = gp.end_date
