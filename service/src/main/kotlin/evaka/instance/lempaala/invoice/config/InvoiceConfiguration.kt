...
package evaka.instance.lempaala.invoice.config
...
class InvoiceConfiguration {
    ...
    fun incomeCoefficientMultiplierProvider(): IncomeCoefficientMultiplierProvider =
        LempaalaIncomeCoefficientMultiplierProvider()

    @Bean fun invoiceProductProvider(): InvoiceProductProvider = LempaalaInvoiceProductProvider()

    @Bean fun invoiceGenerationLogicChooser() = DefaultInvoiceGenerationLogic

    @Bean fun invoiceNumberProvider(): InvoiceNumberProvider = LempaalaInvoiceNumberProvider()
}
...
class LempaalaInvoiceNumberProvider : InvoiceNumberProvider {
    override fun getNextInvoiceNumber(tx: Database.Read): Long =
        tx.createQuery {
                sql(
                    "SELECT max(number) FROM invoice WHERE number >= 54000001 AND number < 5400042258"
                )
            }
