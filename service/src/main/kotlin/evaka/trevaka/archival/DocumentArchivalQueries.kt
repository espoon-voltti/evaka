...
fun Database.Read.getEligibleDecisionCount(decision: Predicate): Int = createQuery {
    sql(
        """
                SELECT count(d.id)
                FROM decision d
...
fun Database.Read.getEligibleFeeDecisionCount(feeDecision: Predicate): Int = createQuery {
    sql(
        """
                SELECT count(fd.id)
...
fun Database.Read.getEligibleVoucherDecisionCount(voucherDecision: Predicate): Int = createQuery {
    sql(
        """
                SELECT count(vvd.id)
