...
import evaka.core.application.ApplicationPreschoolTypeToggle.DAYCARE_ONLY
import evaka.core.application.ApplicationPreschoolTypeToggle.PREPARATORY_DAYCARE
import evaka.core.application.ApplicationPreschoolTypeToggle.PREPARATORY_ONLY
import evaka.core.application.ApplicationPreschoolTypeToggle.PRESCHOOL_CLUB
import evaka.core.application.ApplicationPreschoolTypeToggle.PRESCHOOL_DAYCARE
import evaka.core.application.ApplicationPreschoolTypeToggle.PRESCHOOL_ONLY
...
fun Database.Read.fetchApplicationSummaries(
                ...
                basis.map { applicationBasis ->
                    ...
                    """
                        ...
                        ApplicationBasis.CLUB_CARE -> {
                            PredicateSql { where("was_on_club_care") }
                        }

                        ApplicationBasis.CONTINUATION -> {
                            PredicateSql { where("continuation") }
                        }
    ...
    val orderBy =
        when (sortBy) {
            ...
            ApplicationSortColumn.CHILD_NAME -> {
                "ORDER BY last_name $sortDir, first_name"
            }

            ApplicationSortColumn.DUE_DATE -> {
                "ORDER BY duedate $sortDir, last_name, first_name"
            }
            ...
            ApplicationSortColumn.UNIT_NAME -> {
                "ORDER BY d.name $sortDir, last_name, first_name"
            }
        }.exhaust()

    val applicationSummaries = createQuery {
        sql(
            """
        SELECT
            a.id,
...
): List<PersonApplicationSummary> = createQuery {
    sql(
        """
                SELECT
                    a.id AS applicationId,
...
fun Database.Read.fetchApplicationSummariesForCitizen(
    citizenId: PersonId,
    today: LocalDate,
): List<CitizenApplicationSummary> = createQuery {
...
fun Database.Read.getApplicationType(id: ApplicationId): ApplicationType = createQuery {
    sql("SELECT type FROM application WHERE id = ${bind(id)}")
}
...
fun Database.Transaction.resetCheckedByAdminAndConfidentiality(
    ...
    val confidential =
        when {
            ...
            // If any of these fields are filled, the application is always confidential
            form.otherInfo.isNotBlank() -> {
                null
            }

            // else if other info is filled, the confidentiality is not known
            else -> {
                false
            } // else the application is not confidential
        }
...
fun Database.Read.getApplicationOtherGuardians(id: ApplicationId): Set<PersonId> = createQuery {
    sql(
        """
SELECT guardian_id
...
fun Database.Transaction.removeOldDrafts(clock: EvakaClock) {
    ...
    val applicationIds = createQuery {
        sql(
...
fun Database.Read.getApplicationAttachments(
    applicationId: ApplicationId
): List<ApplicationAttachment> = createQuery {
    sql(
        """
SELECT
...
fun Database.Transaction.cancelActiveTransferApplications(
    ...
    clock: EvakaClock,
    evakaUserId: EvakaUserId,
): List<ApplicationId> = createUpdate {
    sql(
        """
UPDATE application SET
    status = 'CANCELLED',
