...
import evaka.core.absence.getGroupName
import evaka.core.daycare.DaycareMealtimes
...
class NekkuService(
    ...
    fun cleanNekkuOrdersReport(
        dbc: Database.Connection,
        clock: EvakaClock,
        job: AsyncJob.CleanNekkuOrdersReport,
    ) {
...
private fun getNekkuProductNumber(
    ...
    if (filteredNekkuProducts.count() > 1) {
        logger.info {
            "Found too many Nekku Products from database with customertype=$customerType optionsId=${nekkuChildInfo.optionsId} mealtype=${nekkuChildInfo.mealType} mealtime=${nekkuProductMealTime.name}"
        }
        error(
            "Found too many Nekku Products from database with customertype=$customerType optionsId=${nekkuChildInfo.optionsId} mealtype=${nekkuChildInfo.mealType} mealtime=${nekkuProductMealTime.name}"
        )
...
enum class NekkuProductMealTime : DatabaseEnum {
    BREAKFAST,
    LUNCH,
    SNACK,
    DINNER,
