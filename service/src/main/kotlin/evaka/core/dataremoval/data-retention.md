...
Foreign rows are found next. The loader goes through every secondary reference whose target table has loaded rows. The rows that hold the id of one of those rows in the reference column, apart from the own rows, form a foreign node.
