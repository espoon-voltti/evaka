...
import evaka.core.application.getCitizenChildren
...
import evaka.core.shared.domain.formatName
...
LEFT JOIN LATERAL (
    SELECT m.sender_id, m.content_deleted_at
    FROM message m
    WHERE m.thread_id = t.id
    ORDER BY m.sent_at, m.created
    LIMIT 1
) first_msg ON TRUE
...
SELECT
    ...
    END AS recipients,
    CASE
        WHEN m.content_deleted_at IS NOT NULL THEN '[]'::jsonb
        ELSE coalesce((
...
SELECT
    ...
    END AS content,
    CASE
        WHEN msg.is_content_deleted THEN '[]'::jsonb
    ...
    createQuery {
            sql(
                ...
                SELECT
                    ...
                    coalesce(
                        jsonb_agg(
                            jsonb_build_object('id', att.id, 'name', att.name, 'contentType', att.content_type)
                            ORDER BY att.id
