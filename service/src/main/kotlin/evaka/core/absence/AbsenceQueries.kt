...
import evaka.core.attendance.ChildAttendanceRow
import evaka.core.attendance.getChildAttendances
import evaka.core.dailyservicetimes.DailyServiceTimeRow
import evaka.core.dailyservicetimes.DailyServiceTimes
import evaka.core.dailyservicetimes.toDailyServiceTimes
 ...
 * - not generated by questionnaire
 */
...
fun Database.Read.getGroupName(groupId: GroupId): String? = createQuery {
    sql(
        """
...
fun Database.Read.getDaycareIdByGroup(groupId: GroupId): DaycareId = createQuery {
    sql(
        """
SELECT daycare.id
FROM daycare_group
...
fun Database.Read.getAbsences(where: Predicate): List<Absence> = createQuery {
    sql(
        """
SELECT
    a.child_id,
...
fun Database.Read.getAbsenceDatesForChildrenInRange(
    childIds: Set<ChildId>,
    range: FiniteDateRange,
): Map<ChildId, Set<LocalDate>> = createQuery {
    sql(
        """
...
): Map<ChildId, List<FiniteDateRange>> = createQuery {
    sql(
        """
