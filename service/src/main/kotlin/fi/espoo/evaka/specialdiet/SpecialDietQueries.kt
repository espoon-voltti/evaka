...
data class MealTexture(
    @PropagateNull val id: Int?,
    val name: String
)

data class SpecialDiet(
    @PropagateNull val id: Int?,
 ...
 * child.
 */
fun Database.Transaction.resetSpecialDietsNotContainedWithin(specialDietList: List<SpecialDiet>): Map<ChildId, SpecialDiet> {
...
fun Database.Transaction.resetMealTexturesNotContainedWithin(mealTextureList: List<MealTexture>): Int {
...
fun Database.Transaction.getSpecialDiets(): List<SpecialDiet> =
    createQuery {
        sql("SELECT id, abbreviation FROM special_diet")
    }.toList<SpecialDiet>()

fun Database.Transaction.getMealTextures(): List<MealTexture> =
    createQuery {
        sql("SELECT id, name FROM meal_texture")
    }.toList<MealTexture>()
