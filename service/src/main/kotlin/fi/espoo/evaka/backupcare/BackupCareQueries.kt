...
SELECT
  ...
  daycare_group.name AS group_name,
  daterange(backup_care.start_date, backup_care.end_date, '[]') AS period,
  days_in_range(daterange(backup_care.start_date, backup_care.end_date, '[]')) - days_with_service_need AS missingServiceNeedDays
FROM backup_care
JOIN (
  SELECT
    bc.id,
    coalesce(sum(days_in_range(daterange(bc.start_date, bc.end_date, '[]') * sn.period)), 0) AS days_with_service_need
  FROM backup_care bc
  LEFT JOIN (
