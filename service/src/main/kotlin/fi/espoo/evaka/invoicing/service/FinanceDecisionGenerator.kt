    ...
    fun generateNewDecisionsForAdult(
        ...
        val adults =
            if (skipPropagation) {
                ...
                getAllPossiblyAffectedAdultsByAdult(tx, personId)
            }
...
private fun Database.Read.getPartnersFromFinanceDecisions(personId: PersonId) =
    createQuery {
        sql(
            """
        SELECT partner_id FROM fee_decision WHERE head_of_family_id = ${bind(
                personId
            )} AND status NOT IN ('DRAFT', 'IGNORED') AND partner_id IS NOT NULL 
        UNION ALL 
        SELECT head_of_family_id FROM fee_decision WHERE partner_id = ${bind(personId)} AND status NOT IN ('DRAFT', 'IGNORED')
        UNION ALL 
        SELECT partner_id FROM voucher_value_decision WHERE head_of_family_id = ${bind(
                personId
            )} AND status NOT IN ('DRAFT', 'IGNORED') AND partner_id IS NOT NULL 
        UNION ALL 
...
private data class FeeDecisionParents(
...
private fun Database.Read.getChildrenOfHeadOfFamily(
    personId: PersonId,
