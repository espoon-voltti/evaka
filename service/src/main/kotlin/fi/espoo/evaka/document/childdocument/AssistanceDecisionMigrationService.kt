...
@Service
class AssistanceDecisionMigrationService(private val asyncJobRunner: AsyncJobRunner<AsyncJob>) {
    init {
        asyncJobRunner.registerHandler(::migrateDaycareAssistanceDecision)
    }
    ...
    fun migrateDaycareAssistanceDecision(
        ...
        val content = migrateContent(decision, getTranslations(decision.language))
        ...
        db.transaction { tx ->
            ...
            // todo: plan pdf generation if documentKey is null
            // Note: AsyncJob.CreateChildDocumentPdf needs a new parameter to skip
            // sending messages after the pdf upload!
        }
    }
}
...
private fun Database.Transaction.deletePreviouslyMigratedDecision(id: ChildDocumentId) {
    ...
    execute {
        sql(
            """
        DELETE FROM child_document_decision 
        WHERE id IN (SELECT decision_id FROM child_document WHERE id = ${bind(id)})
    """
        )
    }
    execute { sql("DELETE FROM child_document WHERE id = ${bind(id)}") }
}
...
private fun Database.Transaction.insertMigratedDocument(
    ...
    val childDocumentDecisionId =
        ...
        VALUES (${bind(decidedAt)}, ${bind(userId)}, ${bind(decidedAt)}, ${bind(userId)}, $decisionStatus, ${bind(decisionValidity.start)}, ${bind(decisionValidity.end)}, ${bind(decisionNumber)}, ${bind(daycareId)})
        RETURNING id
    """
                )
            }
...
VALUES (${bind(id)}, ${bind(childId)}, ${bind(content)}, ${bind(decidedAt)}, 'COMPLETED', ${bind(content)}, ${bind(decidedAt)}, ${bind(documentKey)}, ${bind(decidedAt)}, ${bind(userId)}, ${bind(processId)}, ${bind(systemUser)}, ${bind(type)}, ${bind(decisionMaker)}, ${bind(childDocumentDecisionId)})
"""
            )
        }
        .updateExactlyOne()
