...
private fun addServiceNeedDataToVarda(
    ...
    try {
        ...
        // need is
        // in effect after evaka started invoicing
        val shouldHaveFeeData =
            if (!db.read { it.serviceNeedIsInvoicedByMunicipality(evakaServiceNeed.id) }) false
            else !evakaServiceNeed.endDate.isBefore(feeDecisionMinDate)
...
private fun vardaFeeBasisByPlacementType(type: PlacementType): String {
    ...
    else FeeBasisCode.DAYCARE.code
}
