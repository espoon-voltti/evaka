...
private typealias GetRolesInPlacementPlanUnit<T> = (tx: Database.Read, employeeId: EmployeeId, targets: Set<T>) -> Iterable<IdAndRole>

data class HasRoleInPlacementPlanUnit(val oneOf: EnumSet<UserRole>) : ActionRuleParams<HasRoleInPlacementPlanUnit> {
    init {
    ...
    override fun merge(other: HasRoleInPlacementPlanUnit): HasRoleInPlacementPlanUnit = HasRoleInPlacementPlanUnit(
        (this.oneOf.asSequence() + other.oneOf.asSequence()).toEnumSet()
    )

    private data class Query<T : Id<*>>(private val getRolesInPlacementUnit: GetRolesInPlacementPlanUnit<T>) :
        DatabaseActionRule.Query<T, HasRoleInPlacementPlanUnit> {
        override fun execute(tx: Database.Read, user: AuthenticatedUser, targets: Set<T>): Map<T, DatabaseActionRule.Deferred<HasRoleInPlacementPlanUnit>> = when (user) {
    ...
    private data class Deferred(private val rolesInUnit: Set<UserRole>) : DatabaseActionRule.Deferred<HasRoleInPlacementPlanUnit> {
        override fun evaluate(params: HasRoleInPlacementPlanUnit): AccessControlDecision = if (rolesInUnit.any { params.oneOf.contains(it) }) {
