...
class EvakaMessageProvider() : IMessageProvider {
    override fun get(type: MessageType, lang: MessageLanguage): String {
        return when (type) {
            MessageType.DECISION_HEADER -> when (lang) {
            ...
            MessageType.DECISION_CONTENT -> when (lang) {
                MessageLanguage.FI ->
...
Vänligen observera att du ska ge ditt svar till beslutet inom två veckor."""
            ...
            MessageType.FEE_DECISION_HEADER -> when (lang) {
                MessageLanguage.FI -> """Espoon varhaiskasvatukseen liittyvät päätökset"""
                MessageLanguage.SV -> """Beslut gällande Esbos småbarnspedagogik"""
            }
            MessageType.FEE_DECISION_CONTENT -> when (lang) {
            ...
            MessageType.VOUCHER_VALUE_DECISION_HEADER -> when (lang) {
                MessageLanguage.FI -> "Espoon varhaiskasvatukseen liittyvät päätökset"
                MessageLanguage.SV -> "Beslut gällande Esbos småbarnspedagogik"
            }
            MessageType.VOUCHER_VALUE_DECISION_CONTENT -> when (lang) {
                MessageLanguage.FI ->
                    """
