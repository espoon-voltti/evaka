...
val feeDecisionRowMapper = RowViewMapper { rv ->
    FeeDecision(
        ...
        children =
            rv.mapColumn<ChildId?>("child_id")?.let { childId ->
                listOf(
                    FeeDecisionChild(
                        child =
                        ...
                        serviceNeed =
                            FeeDecisionServiceNeed(
                                ...
                                contractDaysPerMonth =
                                    rv.mapColumn("service_need_contract_days_per_month"),
...
val feeDecisionDetailedRowMapper = RowViewMapper { rv ->
    FeeDecisionDetailed(
        ...
        partner =
            rv.mapColumn<PersonId?>("partner_id")?.let { id ->
                PersonDetailed(
                    id = id,
        ...
        // child is not nullable so if it's missing there was nothing to join to the decision
        children =
            rv.mapColumn<PersonId?>("child_id")?.let { childId ->
                listOf(
        ...
        documentKey = rv.mapColumn("document_key"),
        approvedBy =
            rv.mapColumn<EmployeeId?>("approved_by_id")?.let { id ->
                EmployeeWithName(
                    id = id,
...
val feeDecisionSummaryRowMapper = RowViewMapper { rv ->
    FeeDecisionSummary(
        ...
        // child is not nullable so if it's missing there was nothing to join to the decision
        children =
            rv.mapColumn<PersonId?>("child_id")?.let { id ->
                listOf(
                    PersonBasic(
                        id = id,
                        dateOfBirth = rv.mapColumn("child_date_of_birth"),
