            ...
            when (ctx.user) {
                is AuthenticatedUser.Employee ->
                    filter(ctx.user, ctx.now).let { subquery ->
                        ctx.tx.createQuery(subquery).mapTo<Id<DatabaseTable>>().toSet().let { ids ->
            ...
            override val query =
                object : DatabaseActionRule.Scoped.Query<EmployeeId, IsEmployee> {
                    override fun executeWithTargets(
                        ...
                        when (ctx.user) {
                            is AuthenticatedUser.Employee ->
                                targets.filter { it == ctx.user.id }.associateWith { Deferred }
                            else -> emptyMap()
                        }
                    ...
                    override fun executeWithParams(
                        ctx: DatabaseActionRule.QueryContext,
                        params: IsEmployee
                    ): AccessControlFilter<EmployeeId>? =
                        when (ctx.user) {
                            is AuthenticatedUser.Employee ->
                                AccessControlFilter.Some(setOf(ctx.user.id))
                            else -> null
                        }
