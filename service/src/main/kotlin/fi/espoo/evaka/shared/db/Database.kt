...
class Database(private val jdbi: Jdbi) {
    ...
    open class Read internal constructor(val handle: Handle) {
        ...
        fun createQuery(fragment: QueryFragment<*>): Query =
            Query(handle.createQuery(fragment.sql)).apply { addBindings(fragment.bindings) }
 ...
 * `Tag` can be used to assign some type to a query fragment for documentation purpose and to
 * prevent mixing different types of query fragments.
 */
data class QueryFragment<@Suppress("unused") Tag>(
    ...
    inline fun <reified T> bind(
        ...
        value: T,
        qualifiers: Array<out KClass<out Annotation>> = defaultQualifiers<T>()
    ): QueryFragment<Tag> = bind(Binding.of(name, value, qualifiers))
