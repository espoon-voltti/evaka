...
fun <T, R> List<T>.pagedForPageSize(
    f: PagedFactory<T, R>,
    total: Int,
    pageSize: Int
): R = f(this, total, if (total % pageSize == 0) total / pageSize else total / pageSize + 1)

fun <T, R> List<WithCount<T>>.mapToPaged(
    f: PagedFactory<T, R>,
    pageSize: Int
): R =
    if (this.isEmpty()) {
...
fun <T, R> Database.Query.mapToPaged(
    ...
    pageSize: Int,
    mapper: Row.() -> T
): R = this.mapToPaged(f, pageSize, "count", mapper)
