...
private typealias GetGroupRoles =
    (user: AuthenticatedUser.Employee, now: HelsinkiDateTime) -> QueryFragment<IdRoleFeatures>
...
data class HasGroupRole(val oneOf: EnumSet<UserRole>, val unitFeatures: Set<PilotFeature>) {
    ...
    private data class Query<T : Id<*>>(private val getGroupRoles: GetGroupRoles) :
        ...
        override fun executeWithTargets(
            ...
            when (ctx.user) {
                is AuthenticatedUser.Employee ->
                    getGroupRoles(ctx.user, ctx.now)
                        .let { subquery ->
                            ctx.tx
                                .createQuery(
                                    QueryFragment<Any>(
        ...
        override fun executeWithParams(
            ...
            when (ctx.user) {
                is AuthenticatedUser.Employee ->
                    getGroupRoles(ctx.user, ctx.now).let { subquery ->
                        ctx.tx
                            .createQuery(
                                QueryFragment<Any>(
                                    """
