...
data class OphQuestion(val name: Map<VasuLanguage, String>, val options: List<OphQuestionOption>)

data class OphQuestionOption(val key: String, val name: Map<VasuLanguage, String>)

fun getDefaultTemplateContent(type: CurriculumType, lang: VasuLanguage) =
    when (type) {
        DAYCARE -> getDefaultVasuContent(lang)
        PRESCHOOL -> getDefaultLeopsContent(lang)
    }

fun getDefaultVasuContent(lang: VasuLanguage) =
    VasuContent(
                            ...
                            VasuQuestion.TextQuestion(
                                name =
                                    when (lang) {
                                        ...
                                        VasuLanguage.SV ->
                                            "Beskrivning av hur målen har uppnåtts och åtgärderna har genomförts"
                                    },
                                info =
                                    when (lang) {
                                        VasuLanguage.FI ->
...
fun getDefaultLeopsContent(lang: VasuLanguage) =
    VasuContent(
        hasDynamicFirstSection = true,
        sections =
            listOf(
                ...
                options =
                    listOf(
                        OphQuestionOption(
                            key = "goal1",
                            name =
                                mapOf(VasuLanguage.FI to "Joku tavoite", VasuLanguage.SV to "mål 1")
                        ),
                        OphQuestionOption(
                            key = "goal2",
                            name =
