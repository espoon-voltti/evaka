...
fun Database.Transaction.upsertAbsences(
    ...
    for (absence in absences) {
        batch.bindKotlin(absence).bind("now", clock.now()).bind("modifiedBy", modifiedBy).add()
    }
...
fun Database.Read.getAbsencesByRange(
    groupId: GroupId,
    range: FiniteDateRange
...
fun Database.Read.getAbsencesByChildByRange(
    childId: ChildId,
    range: FiniteDateRange
): List<Absence> {
    // language=SQL
...
private fun Database.Read.getBackupCaresAffectingGroup(
    groupId: GroupId,
...
private fun Database.Read.getGroupReservations(
    groupId: GroupId,
...
private fun Database.Read.getGroupAttendances(
    groupId: GroupId,
    dateRange: FiniteDateRange
...
private fun Database.Read.getGroupDailyServiceTimes(
    groupId: GroupId,
    dateRange: FiniteDateRange
): Map<ChildId, List<DailyServiceTimesWithId>> =
    createQuery(
            """
WITH all_placements AS (
  $placementsQuery
...
"""
        ...
        .bind("groupId", groupId)
        .bind("dateRange", dateRange)
        .map { rv -> rv.mapColumn<ChildId>("child_id") to toDailyServiceTimes(rv.mapRow()) }
        .toList()
        .groupBy { it.first }
