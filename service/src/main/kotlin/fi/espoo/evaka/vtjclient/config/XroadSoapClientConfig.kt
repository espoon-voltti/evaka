...
class XroadSoapClientConfig {
    ...
    @Bean
    @Profile("vtj-dev")
    fun requestLoggingMarshaller(): Jaxb2Marshaller = Jaxb2Marshaller().apply {
        setPackagesToScan(SOAP_PACKAGES)
    ...
    @Bean
    fun soapFaultResolver(): FaultMessageResolver = XroadFaultMessageResolver()
        ...
        override fun resolveFault(message: WebServiceMessage) {
            when (message) {
                is FaultAwareWebServiceMessage -> {
                    logger.error("Fault while doing X-Road request: ${message.faultCode}. Reason: ${message.faultReason}", message)
    ...
    @Bean
    fun vtjObjectFactory() = ObjectFactory()
    ...
    fun httpsMessageSender(trustManagersFactoryBean: TrustManagersFactoryBean) = HttpsUrlConnectionMessageSender()
        .apply {
            ...
            // TODO: Either keep using single certs or fix the certs and host names for security servers
