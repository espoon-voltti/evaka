...
SELECT
    'https://varhaiskasvatus.tampere.fi/employee/child-information/' || child.id AS child_url,
...
LEFT JOIN fridge_child fc ON (
    fc.child_id = child.id AND
    p1.start_date BETWEEN fc.start_date AND fc.end_date
)
LEFT JOIN person head ON head.id = fc.head_of_child
LEFT JOIN fridge_partner fp1 ON (
    fp1.person_id = head.id AND
    p1.start_date BETWEEN fp1.start_date AND fp1.end_date
)
LEFT JOIN fridge_partner fp2 ON (
    fp1.partnership_id = fp2.partnership_id AND
    p1.start_date BETWEEN fp2.start_date AND fp2.end_date AND
    fp1.indx != fp2.indx
)
LEFT JOIN person partner ON partner.id = fp2.person_id
WHERE
    p1.start_date BETWEEN '2025-08-01'::date AND '2025-10-31'::date AND
    p1.type NOT IN ('PRESCHOOL', 'PRESCHOOL_DAYCARE', 'PRESCHOOL_DAYCARE_ONLY') AND
    (
        EXISTS (
            SELECT 1 FROM placement p2
            JOIN daycare d2 ON d2.id = p2.unit_id
            WHERE
                p1.child_id = p2.child_id AND
                p1.start_date = p2.end_date + interval '1 day' AND
                (d1.provider_type != d2.provider_type OR p1.type != p2.type) AND
                p2.type NOT IN ('PRESCHOOL', 'PRESCHOOL_DAYCARE', 'PRESCHOOL_DAYCARE_ONLY')
         ) OR
         NOT EXISTS (
             SELECT 1 FROM placement p2
