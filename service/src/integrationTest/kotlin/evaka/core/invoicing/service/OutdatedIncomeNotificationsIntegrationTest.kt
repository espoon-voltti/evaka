...
class OutdatedIncomeNotificationsIntegrationTest : FullApplicationTest(resetDbBeforeEach = true) {
    ...
    fun `expiring income is pushed to a guardian who has only a push device`() {
        ...
        assertEquals(0, getEmails().size)
        assertEquals(
            listOf(CitizenPushNotification.Income(IncomeNotificationType.INITIAL_EMAIL)),
    ...
    fun `Final notification is sent after income expires`() {
        ...
        val feeFecisions = db.read {
            it.findFeeDecisionsForHeadOfFamily(fridgeHeadOfChild.id, null, null)
        }
        assertEquals(
            1,
            feeFecisions
