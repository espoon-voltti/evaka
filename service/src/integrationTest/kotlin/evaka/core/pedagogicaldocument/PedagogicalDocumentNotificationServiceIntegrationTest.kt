...
class PedagogicalDocumentNotificationServiceIntegrationTest :
    ...
    @Test
    fun `notification is not pushed when the child's placement has ended`() {
        db.transaction { tx ->
            tx.insertTestCitizenPushSubscription(testGuardianFi.id, mockWebPushEndpoint(httpPort))
        }
        postNewDocument(user = staffUser, PedagogicalDocumentPostBody(child.id, "foobar"))
        db.transaction { tx ->
            tx.execute {
                sql(
                    "UPDATE placement SET end_date = ${bind(LocalDate.now().minusDays(1))} WHERE child_id = ${bind(child.id)}"
                )
            }
        }
        asyncJobRunner.runPendingJobsSync(RealEvakaClock())
