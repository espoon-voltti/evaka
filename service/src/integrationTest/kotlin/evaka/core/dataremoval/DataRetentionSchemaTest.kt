 ...
 *    its own, add the foreign key to [handledByDatabase], or
 * 3) if the decision is left for later, add it to [notYetDeclared]
 */
class DataRetentionSchemaTest : PureJdbiTest(resetDbBeforeEach = false) {
    ...
    /**
     * A reference declared to child may target person until the foreign keys are migrated
     * (chapter 9)
     */
    ...
    private val declaredReferences: Set<ColumnRef> =
        declaredTables
            .flatMap { table -> table.references.map { ColumnRef(table.name, it.column) } }
            .toSet()

    /**
     * Foreign keys of the tables excluded by exception (chapter 5): the database cascade or set
     * null handles their rows, and they need no rule of their own
     */
...
"""
    ...
    fun `every declared reference is a foreign key to the id column of the declared table`() {
        val invalid = declaredTables.flatMap { table ->
            table.references.mapNotNull { reference ->
                val targets =
                    if (reference.toTable == CHILD_TABLE) childTargets else setOf(reference.toTable)
                val idColumn =
                    schema.tablesByName.getValue(reference.toTable).identifiedByCols.single()
                val valid = foreignKeys.any {
                    it.ref == ColumnRef(table.name, reference.column) &&
                        it.referencesRef.tableName in targets &&
                        it.referencesRef.columnName == idColumn &&
                        it.columnCount == 1
                }
                if (valid) null else "${table.name}.${reference.column} -> ${reference.toTable}"
            }
        }
    ...
    /**
     * Every own row of a run holds the target's id in a primary reference to person or child, so
     * the id columns identify the rows of the run when they are unique together with that column
     */
    @Test
    fun `the id columns of every table are its primary key or a unique constraint or index, alone or with a primary reference to the target person`() {
        val invalid =
            declaredTables
                .filter { table ->
                    val primary =
                        (table as? HandledTable)
                            ?.let { schema.primaryReference(it) }
                            ?.takeIf { it.toTable in childTargets }
                    val accepted =
                        listOfNotNull(
                            table.identifiedByCols.toSet(),
                            primary?.let { table.identifiedByCols.toSet() + it.column },
                        )
                    uniqueKeys.none {
                        it.tableName == table.name &&
                            it.predicate == null &&
                            it.columns.toSet() in accepted
                    }
                }
    ...
    fun `the column of an optional reference and its alsoNull columns are nullable`() {
        val invalid = declaredTables.flatMap { table ->
            table.optionalReferences
                .flatMap { reference -> listOf(reference.column) + reference.alsoNull }
                .map { ColumnRef(table.name, it) }
    ...
    fun `every column a date source reads exists with the declared type`() {
        val invalid = handledTables.flatMap { table ->
            table.expirationRule.dateSources().mapNotNull { source ->
    ...
    private fun mayHaveNoDate(table: HandledTable, source: DateSource): Boolean =
        when (source) {
            ...
            is DateSource.Custom -> source.mayHaveNoDate
        }

    /**
     * A table on the primary path reached through NOT NULL reference columns has a row whenever the
     * table has one, except `child`, whose row may be missing (chapter 6)
     */
    private fun isRequiredAncestor(table: HandledTable, ancestor: String): Boolean {
        ...
        while (true) {
            ...
            if (columnOf(ColumnRef(current.name, reference.column))?.nullable != false) return false
            if (reference.toTable == ancestor) return ancestor != CHILD_TABLE
            current = schema.tablesByName.getValue(reference.toTable) as HandledTable
        }
    }
    ...
    fun `every custom date source and archived rule runs against the database`() {
        db.read { tx ->
            for (table in handledTables) {
                for (source in table.expirationRule.customDateSources()) {
                    assertEquals(emptyMap(), source.query(tx, emptyList()))
                }
                for (rule in table.expirationRule.archivedIfRequiredRules()) {
