...
import evaka.core.shared.dev.updateDaycareOperationTimes
...
class AbsenceServiceIntegrationTest : FullApplicationTest(resetDbBeforeEach = true) {
    ...
    fun `calendar has correct absence categories`() {
        val roundTheClockGroupId = db.transaction { tx ->
    ...
    fun `get absence by childId should not find anything with a wrong childId`() {
        ...
        val absences = db.transaction { tx ->
            tx.upsertAbsences(now, employee.evakaUserId, initialAbsenceList)
            getAbsencesOfChildByMonth(tx, otherChild.id, absenceDate.year, absenceDate.monthValue)
        }
    ...
    @Test
    fun `missingHolidayQuestionnaireAnswer is false when child calendar has not opened yet during questionnaire active period`() {
        val questionnaireActive =
            FiniteDateRange(LocalDate.of(2019, 7, 1), LocalDate.of(2019, 7, 15))
        val holidayWeek = FiniteDateRange(LocalDate.of(2019, 8, 5), LocalDate.of(2019, 8, 9))
        val questionnaire =
        ...
        val placement = FiniteDateRange(LocalDate.of(2019, 8, 1), LocalDate.of(2019, 8, 31))
        ...
        val result = db.read {
            getGroupMonthCalendar(
                ...
                8,
                openRangesConfig,
                calendarOpenBeforePlacementDays = 14,
            )
        }
        ...
        holidayDays.forEach { day ->
            ...
            assertFalse(
                childDay?.missingHolidayQuestionnaireAnswer ?: false,
                "Expected missingHolidayQuestionnaireAnswer=false on ${day.date} (child calendar not open during questionnaire)",
            )
        }
    }
