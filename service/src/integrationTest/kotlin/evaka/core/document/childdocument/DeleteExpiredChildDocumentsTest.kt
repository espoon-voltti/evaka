...
class DeleteExpiredChildDocumentsTest : PureJdbiTest(resetDbBeforeEach = true) {
    ...
    @Test
    fun `document_keys of all published versions are returned for each deleted document`() {
        val template =
            insertTemplate(retentionDays = 10, basis = DocumentDeletionBasis.STATUS_TRANSITION)
        val docId =
            insertDocument(
                ...
                publishedVersions =
                    listOf(
                        DevChildDocumentPublishedVersion(
                            ...
                            createdBy = admin.evakaUserId,
                            publishedContent = DocumentContent(emptyList()),
                            documentKey = "s3-key-v1",
                        ),
                        DevChildDocumentPublishedVersion(
                            versionNumber = 2,
                            createdAt = now.minusYears(1).plusMinutes(1),
                            createdBy = admin.evakaUserId,
                            publishedContent = DocumentContent(emptyList()),
                            documentKey = "s3-key-v2",
                        ),
                        // Pending PDF (no key yet) must not leak as a null key into scheduling.
                        DevChildDocumentPublishedVersion(
                            versionNumber = 3,
                            createdAt = now.minusYears(1).plusMinutes(2),
                            createdBy = admin.evakaUserId,
                            publishedContent = DocumentContent(emptyList()),
                            documentKey = null,
                        ),
        ...
        val result = db.transaction { tx -> tx.deleteExpiredChildDocuments(now) }
        assertEquals(1, result.size)
        assertEquals(setOf("s3-key-v1", "s3-key-v2"), result.single().documentKeys.toSet())
        assertDocumentDoesNotExist(docId)
    }

    @Test
    fun `documents with no published versions yield an empty document_keys list`() {
        val template =
            insertTemplate(retentionDays = 10, basis = DocumentDeletionBasis.STATUS_TRANSITION)
        val docId = insertDocument(template, statusModifiedAt = now.minusYears(1))
        ...
        val result = db.transaction { tx -> tx.deleteExpiredChildDocuments(now) }
        assertEquals(1, result.size)
        assertEquals(emptyList(), result.single().documentKeys)
        assertDocumentDoesNotExist(docId)
    }

    @Test
    fun `already externally-archived documents still return their document_keys for S3 cleanup`() {
        // Even though the external archive holds a copy, the eVaka S3 PDF is orphaned once the
        // child_document row is deleted, so DeleteChildDocumentPdf must be scheduled for it.
        val template =
            insertTemplate(
                retentionDays = 10,
                basis = DocumentDeletionBasis.STATUS_TRANSITION,
        ...
        val result = db.transaction { tx -> tx.deleteExpiredChildDocuments(now) }
        assertEquals(1, result.size)
        assertEquals(listOf("archived-doc-key"), result.single().documentKeys)
        assertDocumentDoesNotExist(docId)
    }
