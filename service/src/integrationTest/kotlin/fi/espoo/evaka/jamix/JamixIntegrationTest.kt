...
import fi.espoo.evaka.shared.utils.MockWebServerHelper
...
class JamixIntegrationTest : FullApplicationTest(resetDbBeforeEach = true) {
    ...
    private fun createJamixClient(mockServer: MockWebServerHelper): JamixClient {
        val jamixEnv =
            JamixEnv(
                url = URI(mockServer.baseUrl),
                user = "testuser",
                password = Sensitive("testpass"),
            )
        return JamixClient(jamixEnv, jsonMapper)
    }
    ...
    fun `meal order jobs for daycare groups without customer number are not planned`() {
        useJamixMockServer { mockServer ->
            ...
            planJamixOrderJobs(db, asyncJobRunner, jamixClient, now)
    ...
    fun `orders are sent correctly`() {
        ...
        useJamixMockServer { mockServer ->
            ...
            sendOrders(jamixClient, 88, 888, monday)
            sendOrders(jamixClient, 88, 888, tuesday)
            sendOrders(jamixClient, 99, 999, monday)
            sendOrders(jamixClient, 99, 999, tuesday)

            // Empty orders should not be sent
            sendOrders(jamixClient, 123, 1234, wednesday) // No groups with customer id 123
            sendOrders(jamixClient, 88, 888, wednesday) // No placements on Wednesday

            // Verify request bodies
            val request1 = mockServer.mockServer.takeRequest()
            assertEquals("POST", request1.method)
            assertEquals(request1.path?.contains("mealorders"), true)
            val order1 =
                jsonMapper.readValue(request1.body.readUtf8(), JamixClient.MealOrder::class.java)
            assertEquals(
            ...
            val request2 = mockServer.mockServer.takeRequest()
            assertEquals("POST", request2.method)
            assertEquals(request2.path?.contains("mealorders"), true)
            val order2 =
                jsonMapper.readValue(request2.body.readUtf8(), JamixClient.MealOrder::class.java)
            assertEquals(
                JamixClient.MealOrder(
            ...
            val request3 = mockServer.mockServer.takeRequest()
            assertEquals("POST", request3.method)
            assertEquals(request3.path?.contains("mealorders"), true)
            val order3 =
                jsonMapper.readValue(request3.body.readUtf8(), JamixClient.MealOrder::class.java)
            assertEquals(
                JamixClient.MealOrder(
                    deliveryDate = monday,
                    customerID = 999,
            ...
            assertEquals(3, mockServer.mockServer.requestCount)
        }
    }
    ...
    fun `diet sync generates warning emails`() {
        ...
        useJamixMockServer(specialDiets = specialDiets) { mockServer ->
            ...
            db.transaction { tx ->
                ...
                // Placement today and another in the future -> email is generated only for the
                // current placement
                tx.insert(
                    DevPlacement(
                        childId = child1.id,
