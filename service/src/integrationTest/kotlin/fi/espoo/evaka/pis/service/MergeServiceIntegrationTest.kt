...
class MergeServiceIntegrationTest : FullApplicationTest(resetDbBeforeEach = true) {
    ...
    fun `merging adult moves incomes and sends update event`() {
        ...
        db.transaction {
            it.insert(DevPerson(id = childId))
            it.insert(DevPerson(id = adultId))
            it.insert(DevPerson(id = adultIdDuplicate))
    ...
    fun `merging child moves placements`() {
        ...
        db.transaction {
            it.insert(DevPerson(id = childId))
            it.insert(DevPerson(id = childIdDuplicate))
