...
class InactiveEmployeesRoleResetIntegrationTest : PureJdbiTest(resetDbBeforeEach = true) {
    ...
    @Test
    fun `employee number is reset when last_login is over 45 days ago`() {
        val employeeId =
            db.transaction {
                val employeeId =
                    it.insert(
        ...
        val employeeNumber = db.read { it.getEmployeeNumber(id = employeeId) }
        assertNull(employeeNumber)
    }

    @Test
    fun `employee number is not reset when last_login is less than 45 days ago`() {
        val employeeId =
            db.transaction {
                val employeeId =
                    it.insert(
                        DevEmployee(
                            lastLogin = firstOfAugust2021.minusDays(daysToDeactivation - 1),
                            employeeNumber = "123456",
                        )
                    )
                val areaId = it.insert(DevCareArea())
                ...
                it.setDaycareAclUpdated(
                    unitId,
                    employeeId,
                    firstOfAugust2021.minusDays(daysToDeactivation - 1),
                )
                employeeId
            }
