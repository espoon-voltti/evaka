...
class InactiveEmployeesRoleResetIntegrationTest : PureJdbiTest() {
    ...
    fun `scoped roles are reset when last_login is over 3 months ago`() {
        val employeeId = db.transaction {
            ...
            val unitId = it.insertTestDaycare(DevDaycare(areaId = areaId))
            it.insertDaycareAclRow(daycareId = unitId, employeeId = employeeId, role = UserRole.STAFF)
            it.setDaycareAclUpdated(unitId, employeeId, firstOfAugust2021.minusMonths(3).minusDays(1))
            employeeId
        }
    ...
    fun `scoped roles are not reset when last_login is distant past but daycare acl has been recently updated`() {
        val employeeId = db.transaction {
            val employeeId = it.insertTestEmployee(DevEmployee(lastLogin = firstOfAugust2021.minusDays(1000)))
            val areaId = it.insertTestCareArea(DevCareArea())
            val unitId = it.insertTestDaycare(DevDaycare(areaId = areaId))
            it.insertDaycareAclRow(daycareId = unitId, employeeId = employeeId, role = UserRole.STAFF)
    ...
    fun `scoped roles are not reset when last_login is in distant past but daycare group acl has been recently updated`() {
        val employeeId = db.transaction {
            ...
            it.insertDaycareGroupAcl(daycareId = unitId, employeeId = employeeId, groupIds = listOf(groupId))
...
"""
        ).bind("timestamp", timestamp).bind("unitId", unitId).bind("employeeId", employeeId).execute()
    }
...
"""
        ).bind("timestamp", timestamp).bind("groupId", groupId).bind("employeeId", employeeId).execute()
    }
}
