        ...
        db.transaction { tx ->
            tx.insert(
                DevPlacement(
                    ...
                    unitId = testDaycare.id,
                    type = PlacementType.DAYCARE,
                    startDate = clock.now().toLocalDate().minusMonths(2),
        ...
        db.transaction { tx ->
            tx.insert(
                DevPlacement(
                    ...
                    unitId = testDaycare.id,
                    type = PlacementType.DAYCARE,
                    startDate = clock.now().toLocalDate().plusWeeks(2).plusDays(1),
                    endDate = clock.now().toLocalDate().plusMonths(2),
                )
            )
        }
        val result = systemController.citizenUser(dbInstance(), user, clock, testAdult_1.id)
        ...
        db.transaction { tx ->
            tx.insert(
                DevPlacement(
                    ...
                    unitId = testDaycare.id,
                    type = PlacementType.DAYCARE,
                    startDate = clock.now().toLocalDate().plusWeeks(2),
                    endDate = clock.now().toLocalDate().plusMonths(2),
                )
            )
        }
