...
import fi.espoo.evaka.insertDraftApplication
...
class ApplicationStateServiceIntegrationTests : FullApplicationTest() {
    ...
    fun `sendApplication - preschool has due date same as sent date`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.PRESCHOOL, applicationId = applicationId)
        }
    ...
    fun `sendApplication - daycare has due date after 4 months if not urgent`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.DAYCARE, urgent = false, applicationId = applicationId)
        }
        db.transaction { tx ->
            // when
            service.sendApplication(tx, serviceWorker, applicationId)
    ...
    fun `sendApplication - daycare has due date after 2 weeks if urgent`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.DAYCARE, urgent = true, applicationId = applicationId)
        }
        db.transaction { tx ->
            // when
            service.sendApplication(tx, serviceWorker, applicationId)
    ...
    fun `sendApplication - daycare has not due date if a transfer application`() {
        db.transaction { tx ->
            // given
            val draft = insertDraftApplication(tx.handle, appliedType = PlacementType.DAYCARE, urgent = false, applicationId = applicationId)
    ...
    fun `moveToWaitingPlacement without otherInfo - status is changed and checkedByAdmin defaults true`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, hasAdditionalInfo = false, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `moveToWaitingPlacement with otherInfo - status is changed and checkedByAdmin defaults false`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, hasAdditionalInfo = true, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `moveToWaitingPlacement with maxFeeAccepted - new income has been created`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, guardian = testAdult_5, maxFeeAccepted = true, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `moveToWaitingPlacement with maxFeeAccepted - if application does not have a preferred start date income will not be created`() {
        db.transaction { tx ->
            ...
            upsertIncome(tx.handle, mapper, earlierIndefinite, financeUser.id)
            insertDraftApplication(tx.handle, guardian = testAdult_5, maxFeeAccepted = true, preferredStartDate = null, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `moveToWaitingPlacement - guardian contact details are updated`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.DAYCARE, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `moveToWaitingPlacement - child is upserted with diet and allergies`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.DAYCARE, hasAdditionalInfo = true, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `cancelApplication from SENT - status is changed`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
            // when
    ...
    fun `createPlacementPlan - daycare part-time`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.DAYCARE_PART_TIME, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
            service.moveToWaitingPlacement(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
    ...
    fun `createPlacementPlan - preschool with daycare`() {
        db.transaction { tx ->
            // given
            insertDraftApplication(tx.handle, appliedType = PlacementType.PRESCHOOL_DAYCARE, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
            service.moveToWaitingPlacement(tx, serviceWorker, applicationId)
        }
        db.transaction { tx ->
    ...
    private fun sendDecisionsWithoutProposalTest(
        ...
        // given
        db.transaction { tx ->
            insertDraftApplication(tx.handle, appliedType = PlacementType.PRESCHOOL, guardian = applier, child = child, applicationId = applicationId)
            service.sendApplication(tx, serviceWorker, applicationId)
            service.moveToWaitingPlacement(tx, serviceWorker, applicationId)
            service.createPlacementPlan(
                tx,
