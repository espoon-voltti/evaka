...
class FinanceDecisionGeneratorIntegrationTest : FullApplicationTest() {
    ...
    @Test
    fun `fee decision generation works as expected with a DAYCARE_FIVE_YEAR_OLDS placement`() {
        val placementPeriod = DateRange(LocalDate.of(2019, 1, 1), LocalDate.of(2019, 12, 31))
    ...
    fun `new service need updates existing draft`() {
        ...
        db.transaction { generator.handlePlacement(it, testChild_1.id, serviceNeed.first) }
    ...
    fun `deleted service need that split existing decision merges decisions afterwards`() {
        ...
        db.transaction { tx ->
            ...
            generator.handlePlacement(tx, testChild_1.id, serviceNeed.first)
        }
