...
class FeeDecisionIntegrationTest : FullApplicationTest() {
    ...
    fun `search works with distinctions param UNCONFIRMED_HOURS`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, listOf(testDecision, testDecisionMissingServiceNeed)) }
    ...
    fun `search works with distinctions param EXTERNAL_CHILD`() {
        ...
        jdbi.handle { h ->
            upsertFeeDecisions(h, objectMapper, listOf(testDecisions[0], testDecisions[3], testDecisionWithExtChild))
        }
    ...
    fun `search works with distinctions param RETROACTIVE`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, listOf(oldDecision, futureDecision)) }
    ...
    fun `search works as expected with decision number as search term`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, listOf(sentDecision, otherDecision)) }
    ...
    fun `confirmDrafts returns bad request when some decisions being in the future`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, listOf(draftWithFutureDates)) }
    ...
    fun `confirmDrafts updates conflicting sent decision end date on confirm`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, listOf(draft, conflict)) }
    ...
    fun `confirmDrafts updates completely overlapped conflicting decisions to annulled state`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, listOf(draft, conflict_1, conflict_2)) }
    ...
    fun `confirmDrafts updates conflicting sent decision to annulled even when the new decisions cover it exactly`() {
        ...
        jdbi.handle { h -> upsertFeeDecisions(h, objectMapper, splitDrafts + conflict) }
    ...
    fun `fee decisions can be forced to be sent manually`() {
        ...
        jdbi.handle { h ->
            h.createUpdate("update person set force_manual_fee_decisions = true where id = :id")
