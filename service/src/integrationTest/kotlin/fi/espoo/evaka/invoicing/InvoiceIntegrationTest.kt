...
class InvoiceIntegrationTest : FullApplicationTest(resetDbBeforeEach = true) {
    ...
    private fun deserializeListResult(json: String) =
        jsonMapper.readValue<Paged<InvoiceSummary>>(json)
    private fun deserializeResult(json: String) =
        jsonMapper.readValue<Wrapper<InvoiceSummary>>(json)
    ...
    fun `search works with canceled status parameter`() {
        ...
        assertEqualEnough(canceled.map(::toSummary), deserializeListResult(result.get()).data)
    }
    ...
    fun `search works with all status parameters`() {
        ...
        assertEqualEnough(invoices.map(::toSummary), deserializeListResult(result.get()).data)
    }
    ...
    fun `search works as expected with multiple partial search terms`() {
        ...
        assertEqualEnough(testInvoices.map(::toSummary), deserializeListResult(result.get()).data)
    }
    ...
    fun `search gives correct total and page composition when using filters`() {
        ...
        assertEqualEnough(sent.map(::toSummary).take(2), deserializeListResult(result.get()).data)
    }
