...
import evaka.instance.espoo.archival.calculateNextJuly31
import evaka.instance.espoo.archival.createCaseFile
import evaka.instance.espoo.archival.createCreation
import evaka.instance.espoo.archival.createDocumentDescription
import evaka.instance.espoo.archival.createDocumentMetadata
import evaka.instance.espoo.archival.createRetentionPolicy
import evaka.instance.espoo.archival.model.PersonalData
...
class DocumentMetadataUtilsTest {
    ...
    private fun createTestDocument(
        ...
        return ChildDocumentDetails(
            ...
            modifiedAt = HelsinkiDateTime.of(LocalDateTime.parse("2023-02-01T12:10:00")),
    ...
    fun `createDocumentDescription sets correct document type and specifier for VASU`() {
        ...
        assertEquals(PersonalData.CONTAINS_SENSITIVE_PERSONAL_INFORMATION, result.personalData)
    ...
    fun `createDocumentDescription includes agents when caseProcess is provided`() {
        ...
        assertEquals("Testi Testaaja", result.agents[0].name)
        assertEquals(
            "Espoon kaupungin esiopetus ja varhaiskasvatus",
            result.agents[0].corporateName,
        )
        assertEquals("Henkilökunta", result.agents[0].role)
    }
    ...
    fun `createCaseFile uses completed state from history when available`() {
        ...
        assertEquals(documentMetadata.createdAtDate, result.caseCreated)

        assertEquals(completionDateTime.toLocalDate(), result.caseFinished)
    }
    ...
    fun `createCaseFile falls back to template end date when no completed state is available`() {
        ...
        assertEquals(document.template.validity.end, result.caseFinished)
    }
    ...
    fun `createCaseFile calculates next July 31 when no other date is available`() {
        ...
        assertEquals(LocalDate.parse("2023-07-31"), result.caseFinished)
    }
    ...
    fun `createCaseFile calculates next July 31 correctly for dates after July 31`() {
        ...
        assertEquals(LocalDate.parse("2024-07-31"), result.caseFinished)
    }
    ...
    fun `createDocumentMetadata sets disclosure policy timeSpan based on confidentiality duration`() {
        ...
        val timeSpan = disclosurePolicy.rule.timeSpan
        assertEquals(confidentialityDuration.toShort(), timeSpan)

        // Verify the confidentiality basis is correctly set
        val actionAnnotation = disclosurePolicy.rule.action.actionAnnotation
    ...
    fun `createRetentionPolicy sets correct policy for pedagogical assessment`() {
        ...
        assertEquals("RetentionPolicy", result.policyName)
    ...
    fun `createCreation prefers case process initial date when available`() {
        ...
        assertEquals(caseProcessInitialDate.toLocalDate(), result.created)
    }
    ...
    fun `createCreation falls back to document metadata when no case process`() {
        ...
        assertEquals(documentMetadata.createdAtDate, result.created)
    }
    ...
    fun `createCaseFile uses case process initial date for caseCreated when available`() {
        ...
        // Should use case process initial date, not document metadata creation date
        assertEquals(caseProcessInitialDate.toLocalDate(), result.caseCreated)
    }
    ...
    fun `getCaseFinishDate uses case process initial date in fallback calculation when available`() {
        ...
        assertEquals(LocalDate.parse("2020-07-31"), result.caseFinished)
    }
}
