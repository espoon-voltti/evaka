...
class SchemaDefinitionTest {
    ...
    fun `the primary references form a root-first tree`() {
        ...
        val primary = { name: String -> schema.primaryReference(schema.handled(name))?.column }
        ...
        val secondary = { name: String ->
            schema.tablesByName.getValue(name).secondaryReferences.map { it.column }
        }
        ...
        for (table in order) {
            val parent =
                primary(table)?.let {
                    schema.handled(table).blockingReferences.first { r -> r.column == it }.toTable
                }
            if (parent != null)
        ...
        assertEquals(
            ...
            schema.primaryPath(schema.handled("fee_decision_child")).map { it.name },
        )
    }
                ...
                HandledTable(
                    ...
                    references =
                        listOf(
                            ...
                            optionalReference("child_id", toTable = "child"),
                        ),
    ...
    fun `every handled table other than person declares exactly one primary reference, whose target has the table's handler`() {
        invalid("a declares no primary reference") {
            schema(
                HandledTable(
                    "a",
                    CHILD,
                    references = listOf(secondaryReference("child_id", toTable = "child")),
                    expirationRule = Always,
                )
            )
        }
        invalid("a declares 2 primary references, exactly one is required") {
            schema(
                HandledTable(
                    ...
                    references =
                        listOf(
                            primaryReference("head_id", toTable = "person"),
                            primaryReference("partner_id", toTable = "person"),
                        ),
                    expirationRule = Always,
                )
            )
        ...
        invalid("x is an external table, which declares no primary reference") {
            schema(ExternalTable("x", listOf(primaryReference("person_id", toTable = "person"))))
        }
        val schema =
            schema(
                table("a", CHILD, "child_id" to "child"),
                HandledTable(
                    ...
                    references =
                        listOf(
                            secondaryReference("a_id", toTable = "a"),
                            primaryReference("other_id", toTable = "a"),
                        ),
                    expirationRule = Always,
                ),
            )
        assertEquals("other_id", schema.primaryReference(schema.handled("b"))?.column)
        assertEquals(listOf("a_id"), schema.handled("b").secondaryReferences.map { it.column })
    }
    ...
    fun `an external table is never loaded as own rows, and no reference leads to it`() {
        ...
        assertEquals(
            listOf("person_id"),
            schema.tablesByName.getValue("message_account").secondaryReferences.map { it.column },
        )
        ...
        invalid("a: column account_id references external table message_account") {
            schema(
                ...
                table(
                    ...
                    CHILD,
                    "child_id" to "child",
                    optional = listOf(optionalReference("account_id", toTable = "message_account")),
                ),
            )
        }
    }
    ...
    fun `optional references are found from their target`() {
        ...
        assertEquals(
            ...
            schema.optionalReferencesInto(application).map { it.table.name to it.reference },
        )
    ...
    fun `a rule safe for integrations is validated, and its sources and archived rules found, through the wrapper`() {
        ...
        assertEquals(setOf(source, OwnColumn("created")), rule.dateSources())
        assertEquals(setOf(archived), rule.archivedIfRequiredRules())
    ...
    fun `the declared schema definition is valid and loads every table after the one its primary reference points to`() {
        for (schema in declaredSchemas) {
            ...
            for (table in order) {
                val parent = schema.primaryReference(table)?.toTable ?: continue
