...
internal fun table(
    ...
    HandledTable(
        ...
        references =
            listOfNotNull(
                primary?.let { primaryReference(it.first, referencedTable = it.second) }
            ) +
                secondary.map { secondaryReference(it.first, referencedTable = it.second) } +
                optional,
        bundledBy = bundledBy,
...
internal fun externalTable(
    ...
    ExternalTable(
        name,
        secondary.map { secondaryReference(it.first, referencedTable = it.second) } + optional,
    )
...
internal fun SchemaDefinition.ownNode(table: String, vararg rows: OwnRow) =
...
internal fun SchemaDefinition.ownRowNode(table: String, row: OwnRow) =
 ...
 * delete no orphan, so a test row must give them all like the loader does
 */
...
/** The loader forms foreign nodes only through secondary references, so a test one must too */
internal fun SchemaDefinition.foreign(
    ...
    require(declared.secondaryReferences.any { it.referenceColumn == column }) {
