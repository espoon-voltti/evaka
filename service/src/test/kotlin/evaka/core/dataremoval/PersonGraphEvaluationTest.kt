...
class PersonGraphEvaluationTest {
    ...
    private fun single(table: String, id: UUID) =
        OwnNodeId.SingleRow(table, RowId(mapOf("id" to id)))
    ...
    /**
     * Whether the node of table a with the given rows is deleted in a graph of person, child and a
     */
    private fun expired(
        rule: ExpirationRule,
        vararg rows: LoadedRow = arrayOf(row("child_id" to target)),
    ): Boolean {
    ...
    fun `a row bundles only the rows whose primary path leads to it`() {
        ...
        assertEquals(
            ...
            graph.bundledOwnNodeIdsByBundler,
        )
    ...
    fun `a rule safe for an integration expires only when every child concerned is never sent to it or past the safe age`() {
        ...
        fun expired(
            ...
            row: LoadedRow = row("child_id" to target),
        ): Boolean {
            val schema = schemaWith(rule)
            val graph =
                schema.graph(
    ...
    @Test
    fun `a hand-written row must give every outside graph reference of its table`() {
        val schema =
            schema(
                table(
                    "document",
                    CHILD,
                    "child_id" to "child",
                    orphansToDelete = listOf(OutsideGraphReference("decision_id", "decision")),
                )
            )
        val error =
            assertFailsWith<IllegalArgumentException> {
                schema.ownNode("document", row("child_id" to target))
            }
        assertContains(error.message!!, "decision_id")
    }
    ...
    @Test
    fun `a hand-written row must give every reference of its table`() {
        val error =
            assertFailsWith<IllegalArgumentException> {
                blockingSchema.ownNode("a", row(id = target))
            }
        assertContains(error.message!!, "[child_id]")
    }
}
