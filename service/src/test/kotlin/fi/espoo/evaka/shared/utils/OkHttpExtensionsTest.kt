...
class OkHttpExtensionsTest {
    ...
    @Test
    fun `executeWithRetries invokes error callback for non-429 errors`() {
        mockWebServer.enqueue(MockResponse().setResponseCode(500).setBody("Server error"))

        val request = Request.Builder().url(mockWebServer.url("/test")).build()
        var callbackInvoked = false
        var callbackRemainingTries = -1

        val response =
            client.executeWithRetries(
                request,
                remainingTries = 5,
                errorCallback = { response, remaining ->
                    callbackInvoked = true
                    callbackRemainingTries = remaining
                    response
                },
            )

        assertTrue(callbackInvoked)
        assertEquals(4, callbackRemainingTries) // 5 - 1
