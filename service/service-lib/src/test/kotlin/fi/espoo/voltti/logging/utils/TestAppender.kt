        ...
        events
            ...
            .let { args ->
                StringWriter().use { sw ->
                    ...
                    JsonFactory().createGenerator(ObjectWriteContext.empty(), pw).use { generator ->
                        generator.run {
                            writeStartObject()
                            args.writeTo(this)
                            writeEndObject()
...
fun KLogger.getTestAppender(): TestAppender =
    (this.underlyingLogger()).getAppender(appenderName) as TestAppender
