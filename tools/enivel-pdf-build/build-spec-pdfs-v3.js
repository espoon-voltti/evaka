...
const CSS = `
  ...
  h1 {
    color: #2c5fa1;
    border-bottom: 2.5pt solid #2c5fa1;
    padding-bottom: 6pt;
    font-size: 26pt;
    margin: 0 0 16pt 0;
    font-weight: 700;
  ...
  h2 {
    ...
    margin-top: 20pt;
    margin-bottom: 8pt;
    border-bottom: 1pt solid #2c5fa1;
    padding-bottom: 3pt;
    font-weight: 700;
    page-break-after: avoid;
  }
...
function renderPage(md, title, headerLeft) {
  return `<!DOCTYPE html>
<html lang="fi">
<head>
  <meta charset="utf-8">
...
function headerTemplate(left) {
  return `<div style="font-size:8pt; width:100%; padding:0 18mm; color:#6e6e6e; font-family:-apple-system,sans-serif; display:flex; justify-content:space-between;">
    <span>${left}</span>
    <span>eNivel</span>
  </div>`;
}
...
(async () => {
  ...
  try {
    ...
    const bodyMd = CHAPTERS.map(c => `\n\n<!-- PAGEBREAK -->\n\n` + fs.readFileSync(path.join(SPEC_DIR, c.file), 'utf8'))
      .join('');
    const combined = indexMd + bodyMd;
    const combinedHtml = renderPage(combined, 'eNivel — Järjestelmämäärittely', 'eNivel — Järjestelmämäärittely');
    ...
    await page.pdf({
      ...
      headerTemplate: headerTemplate('eNivel-määrittely'),
