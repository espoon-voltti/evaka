 ...
 */
export function toIncomeStatementAttachments(attachments) {
    const [attachmentsWithType, untypedAttachments] = partition(attachments, (a) => a.type !== null);
    if (untypedAttachments.length !== 0) {
        return { typed: false, untypedAttachments };
    }
    const attachmentsByType = groupBy(attachmentsWithType, 'type');
    return { typed: true, attachmentsByType };
}
export function numAttachments(incomeStatementAttachments) {
...
export function collectAttachmentIds(incomeStatementAttachments) {
