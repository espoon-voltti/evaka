...
/**
* Generated from fi.espoo.evaka.nekku.NekkuController.getNekkuSpecialDietChoices
*/
export async function getNekkuSpecialDietChoices(
  ...
  const { data: json } = await client.request<JsonOf<NekkuSpecialDietChoices[]>>({
    url: uri`/employee/nekku/special-diet-choices/${request.childId}`.toString(),
    method: 'GET'
  })
  return json
}
