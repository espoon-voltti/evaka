...
describe('Finance basics', () => {
  ...
  test('Preschool club fees section is hidden', async () => {
    await nav.openAndClickDropdownMenuItem('finance-basics')
    await financeBasicsPage.feesSection.createFeeThresholdsButton.click()
    await financeBasicsPage.feesSection.editor.preschoolClubFee.waitUntilHidden()
    await financeBasicsPage.feesSection.editor.preschoolClubSiblingDiscount.waitUntilHidden()
  })
...
describe('Finance basics with preschool club enabled', () => {
  beforeEach(async () => {
    const financeAdmin = await Fixture.employeeFinanceAdmin().save()

    page = await Page.open({
      acceptDownloads: true,
      employeeCustomizations: { placementTypes: ['PRESCHOOL_CLUB'] }
    })
    await employeeLogin(page, financeAdmin.data)
    await page.goto(config.employeeUrl)
  ...
  test('Create a new set of retroactive fee thresholds', async () => {
    const { data } = Fixture.feeThresholds().with({
      ...
      preschoolClubSiblingDiscount: 0.4
    })
    await nav.openAndClickDropdownMenuItem('finance-basics')
  ...
  test('Creating fee thresholds disables save button when preschool club fee is not set', async () => {
    const { data } = Fixture.feeThresholds().with({
  ...
  test('Creating fee thresholds disables save button when preschool club sibling discount is not set', async () => {
    const { data } = Fixture.feeThresholds().with({
  ...
  test('Copying existing fee thresholds', async () => {
    const { data: originalData } = Fixture.feeThresholds().with({
      preschoolClubFee: 14000,
      preschoolClubSiblingDiscount: 0.4
    })
    await insertFeeThresholds(originalData)
  ...
  test('Editing existing fee thresholds', async () => {
    ...
    await thresholdsItem.edit()
    await financeBasicsPage.feesSection.editor.preschoolClubFee.fill('150')
    await financeBasicsPage.feesSection.editor.preschoolClubSiblingDiscount.fill(
      '50'
    )
    await financeBasicsPage.feesSection.editor.save(true)
    ...
    await thresholdsItem.assertItemContains({
      ...originalData,
      preschoolClubFee: 15000,
      preschoolClubSiblingDiscount: 0.5
    })
  })
})
