...
test.describe('Reservation calendar child date modal', () => {
  let daycareId: UUID
  let groupId: UUID
  let childId: UUID
  ...
  async function navigateToTestView(): Promise<UnitWeekCalendarPage> {
  ...
  test('Add absences for child in preschool daycare for tomorrow then update one and remove other', async () => {
    ...
    // defaults from the other absence
    await modal.nonbillableAbsenceType.assertSelectedOption('SICKLEAVE')
  ...
  test.describe('Intermittent shift care', () => {
    test.use({
      evakaOptions: {
        mockedTime: today.toHelsinkiDateTime(LocalTime.of(12, 0)),
