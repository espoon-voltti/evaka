...
describe('Daily service times in the unit calendar', () => {
  // absence-cell-6ec99620-9ffd-11ea-bb37-0242ac130002-2023-02-12
  beforeEach(async () => {
  ...
  const assertOtherAbsenceOn = async (date: LocalDate) =>
    await calendarPage.assertAbsence(child1Fixture.id, date, 'OTHER_ABSENCE')

  test('Fridays and weekends are marked as off for child with irregular service times and Fridays and weekends off', async () => {
    calendarPage = await loadUnitCalendarPage()
    const feb = LocalDate.of(2023, 2, 1)
    await assertOtherAbsenceOn(feb.withDate(3))
    await assertOtherAbsenceOn(feb.withDate(4))
    await assertOtherAbsenceOn(feb.withDate(5))
    await assertOtherAbsenceOn(feb.withDate(10))
    await assertOtherAbsenceOn(feb.withDate(11))
    await assertOtherAbsenceOn(feb.withDate(12))
    await assertOtherAbsenceOn(feb.withDate(17))
    await assertOtherAbsenceOn(feb.withDate(18))
    await assertOtherAbsenceOn(feb.withDate(19))
    await assertOtherAbsenceOn(feb.withDate(24))
    await assertOtherAbsenceOn(feb.withDate(25))
    await assertOtherAbsenceOn(feb.withDate(26))
  })
})
