  ...
  test.describe(`Citizen calendar discussion surveys (${env})`, () => {
    ...
    test('Citizen can reserve a discussion time', async () => {
      const reservationModal =
        await calendarPage.openDiscussionReservationModal(
          groupEventId,
          testChild.id
        )
...
for (const env of ['desktop', 'mobile'] as const) {
  test.describe(`Citizen calendar event time visibility (${env})`, () => {
    let calendarPage: CitizenCalendarPage
    ...
    test('Citizen receives only one correct set of event times despite 2 placements', async ({
      ...
      calendarPage = await loginAndOpenCalendar(newEvakaPage, testAdult2)
