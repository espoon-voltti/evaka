...
test.describe('Citizen push notifications', () => {
  ...
  test('the suggestion shows recovery steps when notifications are blocked', async ({
    evaka
  }) => {
    ...
    const suggestion = evaka.findByDataQa('push-suggestion')
    await expect(suggestion).toHaveAttribute('data-status', 'blocked')
    await expect(
      evaka.findByDataQa('push-suggestion-instructions')
    ).toContainText('Toimi näin')

    await evaka.findByDataQa('push-suggestion-close').click()
