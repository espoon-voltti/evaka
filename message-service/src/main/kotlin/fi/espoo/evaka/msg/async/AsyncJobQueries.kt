...
fun <T : AsyncJobPayload> Handle.insertJob(jobParams: JobParams<T>): UUID = createUpdate(
    // language=SQL
    """
...
fun Handle.getPendingJobCount(jobTypes: Collection<AsyncJobType> = AsyncJobType.values().toList()): Int =
    createQuery(
        // language=SQL
        """
SELECT count(*)
...
fun Handle.claimJob(jobTypes: Collection<AsyncJobType> = AsyncJobType.values().toList()): ClaimedJobRef? =
    createUpdate(
        // language=SQL
        """
WITH claimed_job AS (
...
fun <T : AsyncJobPayload> Handle.startJob(job: ClaimedJobRef, payloadClass: Class<T>): T? = createUpdate(
    // language=SQL
    """
WITH started_job AS (
  SELECT id
...
fun Handle.completeJob(job: ClaimedJobRef) = createUpdate(
    // language=SQL
    """
UPDATE async_job
